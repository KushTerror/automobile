jQuery(document).ready(function ($) {
    loadMasterData("min-year", function () {
...
function loadMasterData(selectId, callBack) {
    var select = $("#" + selectId);
    if (select) {
        getServerData("masters/years", function (result) {
            console.log(result);
            var choices = [{id: 1, year: "2015"}, {id: 2, year: "2014"}, {id: 3, year: "2013"}, {id: 4, year: "2012"}];
            select.find('option').remove().end().append($("<option/>").attr("value", '').text("Min Year"));
            $.each(choices, function (a, b) {
                select.append($("<option/>").attr("value", b.id).text(b.year));
            });
            select.val('');
            callBack();
...
function getServerData(url, okCallBack, errCallBack) {
    jQuery.ajax({
        url: "/rest/" + url,
        type: "GET",
        contentType: 'application/json; charset=utf-8',
        success: function (resultData) {
            okCallBack(resultData);
        },
